...
Physics layer list: This is the physics layer list that you can assign to each tile. Note that this only affects the tileset, and not the tilemap layer node. You can assign each physics layer you have here to a physics layer in Godot. They are separate, so you wire this up with the "Edit physics layer..." button, which lets you tick which of Godot's 32 project-wide 2D collision layers this physics layer occupies (collision layer) and which ones it detects (collision mask). Those 32 slots are numbered, not named, until you name them: use "Name project collision layers..." to give them names (written to Project Settings), after which the list reads e.g. "Physics layer 0 - on: World; scans: World, Enemies". Naming them here is the same as naming them in Project Settings > Layer Names, so the names show up everywhere else in the engine too. For more info about physics layers, please do read the Godot documentation. You have buttons to add a new physics layer here, or remove the selected one. Different layers are useful if you want only specific entities to be able to collide with specific tiles. 
